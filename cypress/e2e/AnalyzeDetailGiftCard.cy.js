describe('Lấy thống kê chi tiết gift-card', function() {
    const accessToken = 'eyJhbGciOiJIUzI1NiIsInR5cCI6IkpXVCJ9..nq-PPy3qakXT-nmbJKqciDZXKnPfv07oP7PMRlEgkmA';

    it('Gift-card đã tồn tại', () => {

        cy.request({
            method: 'GET',
            url: 'api/v1/partner/gift-card/analyze?type=hotel&code=END_YEAR_HOTEL',
            failOnStatusCode: false,
            headers: {
                Authorization: `Bearer ${accessToken}`
            }
        }).should(({ status, body }) => {
            expect(status).to.eq(200);
            expect(body.status).to.eq('success');
            expect(body.message).to.eq('Lấy thông tin thành công !');
            expect(body.data.totalAmount).to.eq(0);
            expect(body.data.totalUsed).to.eq(0);

        })
    });

    it('Gift-card chưa tồn tại', () => {

        cy.request({
            method: 'GET',
            url: 'api/v1/partner/gift-card/analyze?type=hotel&code=END_YEAR_HOTEL12',
            failOnStatusCode: false,
            headers: {
                Authorization: `Bearer ${accessToken}`
            }
        }).should(({ status, body }) => {
            expect(status).to.eq(400);
            expect(body.status).to.eq('fail');
            expect(body.message).to.eq('Thẻ quà tặng không tồn tại !');
        })
    });

});
