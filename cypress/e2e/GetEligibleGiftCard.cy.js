describe('Lấy danh sách gift-card khả dụng', function() {
    ...
    it('Danh sách gift-card hợp lệ của dịch vụ chuyến bay', () => {

        cy.request({
            method: 'GET',
            url: 'api/v1/user/gift-card/eligible?typeVoucher=FLIGHT',
    ...
    it('Danh sách gift-card của dịch vụ đưa đón sân bay nhưng không có gift-card nào', () => {
        cy.request({
            method: 'GET',
            url: 'api/v1/user/gift-card/eligible?typeVoucher=AIRPORT',
            failOnStatusCode: false,
            headers: {
                Authorization: `Bearer ${accessToken}`
            }
    ...
    it('Danh sách gift-card của dịch vụ không tồn tại', () => {
        cy.request({
            method: 'GET',
            url: 'api/v1/user/gift-card/eligible?typeVoucher=FLIGHT232323',
            failOnStatusCode: false,
            headers: {
                Authorization: `Bearer ${accessToken}`
            }
